    ...
    public class UpdateProductDtoValidator : AbstractValidator<UpdateProductDto>
    {
        ...
        public UpdateProductDtoValidator(IProductRepository productRepository)
        {
            ...
            RuleFor(x => x.ProductName)
                ...
                .MustAsync(async (dto, productName, cancellationToken) =>
                {
                    var existingProduct = await _productRepository.GetQueryable()
                        .Where(p => p.ProductName == productName && p.Id != dto.Id)
            ...
            RuleFor(x => x.ProductBarcode)
                ...
                .MustAsync(async (dto, productBarcode, cancellationToken) =>
                {
                    ...
                    var existingProduct = await _productRepository.GetQueryable()
                        .Where(p => p.ProductBarcode == productBarcode && p.Id != dto.Id)
                        .FirstOrDefaultAsync(cancellationToken);
                    return existingProduct == null;
                }).WithMessage("Product barcode already exists.");
