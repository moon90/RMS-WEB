...
| `RMS.Tests/` | xUnit unit test suite (`TableReservationServiceTests`, `StripePaymentServiceTests`, `ReservationConfirmedConsumerTests`) |
...
- MassTransit outbox consumers (`ReservationConfirmedConsumer.cs`) handle domain events asynchronously without blocking HTTP request threads.
